...
An important further result about Lie derivatives is Cartan's formula $L_X = di_X + i_Xd$. (That is, $L_X\omega = d(i_X\omega) + i_x(d\omega)$.) We note first that, since both sides are linear, we need only check it for individual terms of the form (in coordinates) $f dx^1 \wedge \dots \wedge dx^k$. Also, $L_X$ is a derivation, as is the right-hand side: it is true in general that, if $A, B$ are skew-derivations, then $AB + BA$ is a derivation, and $d, i_X$ are both skew-derivations. Finally, both sides commute with $d$. All this means that we can apply both sides to a form, repeatedly use linearity, the product rule, and the fact that both operators commute with $d$, and end up with expressions that depend only on the values of $L_X, di_X + i_Xd$ on scalar functions. In other words, we only have to verify the formula for $0$-forms, and then it will follow for general $k$-forms. 

On one hand $L_Xf = Xf$; on the other hand, $(di_X + i_Xd)f = d(i_Xf) + df(X) = 0 + Xf = Xf$. (N.B. it is true for any scalar function and vector field that $i_Xf = 0$.) Thus the formula holds for 0-forms, and by the argument above it holds for arbitrary differential forms. 